...
- **`_decisions.jsonl`** — ≥1 dòng `"type":"design"` cho mỗi ADR / error-code mapping / API contract
  ...
  `decision`/`reasoning` ngắn kiểu keyword. Format:
...
- **`_glossary.md`** — APPEND row cho thuật ngữ bạn định nghĩa (pattern kiến trúc, tên service,
  chiến lược lock/concurrency…), Phase=S3.
...
- **`_state.json`** — **never rewrite cả file.** MỘT lệnh `node .kiro/tools/state-set.mjs`:
  `--append phase_history='{"phase":"S3","agent":"architect","date":"…","note":"…(1-3 câu; chi tiết → _handoff.md)"}'`
  `--set current_phase=S3 --set last_agent=architect` + `active_concerns` (vùng risk cho developer),
  `terminology` (merge analyst + mới), `next_action` → `agent:"sdlc"`, `command:"approve s3"`,
