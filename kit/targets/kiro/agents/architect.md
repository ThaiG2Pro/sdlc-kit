...
- **`_state.json`**: **never rewrite the whole file.** Một lệnh:
  `node .kiro/tools/state-set.mjs --append phase_history='{"phase":"S3","agent":"architect","date":"…","note":"…(1-3 câu)"}'`
  kèm `--set` cho `active_concerns`/`terminology`/`priority_reading`/`watch_items`
