...
> phân tích/bằng chứng/before-after → **dev-test-report.md** (đọc 1 lần ở BUILD gate); baton chỉ giữ
> kết luận 1 dòng + con trỏ. Cap: `_handoff.md` ≤6 KB, `_glossary.md` ≤6 KB, `_progress.md` ≤4 KB,
...
- **`_decisions.jsonl`** — ≥1 entry `"type":"implementation"` (hoặc `"deviation"`) cho mỗi deviation +
  ...
  Write khi hết checkpoint (không phải 1 Write/quyết định). Append-only. **Cap**: `decision` ≤240 ký
  tự, `reasoning`/`rejected` ≤120 — keyword/fragment. Format:
...
- **`_glossary.md`** — row chỉ khi định nghĩa thuật ngữ kỹ thuật mới mà QA không đọc ra được từ code/
  design (1 dòng ≤220 ký tự). Nghĩa đổi → sửa row cũ tại chỗ, không thêm row `[SUPERSEDED]`.
- **`_handoff.md`** — OVERWRITE (shape: `handoff-template.md`), header `Generated by: developer`
  ...
  **THAY toàn bộ, đúng 5 section.** Vòng S4-FIX sau **viết lại chính 5 section đó** cho round hiện
  tại — không thêm `## Round N`/`## Addendum` (git giữ round trước).
...
- **`_state.json`** — **never rewrite cả file.** Mỗi checkpoint: `node .kiro/tools/state-set.mjs --set
  ...
  `--append phase_history='{"phase":"S4","agent":"developer","date":"…","note":"…(1-2 câu, ≤200 ký tự)"}'
  ...
  trước", tốn 1 lượt gọi vô ích. Cũng đừng pipe (`| tail`, `| head`) — chaining thật, phá luôn
  exception (bạn được phép chạy script vì viết code, nhưng state-set vẫn là
