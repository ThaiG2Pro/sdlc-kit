...
4. **Context** — `context/stack.md` (lệnh build/test/lint/coverage THẬT — không bao giờ đoán),
   ...
   `context/{project,architecture,conventions,stack}.md` + steering `sdlc-workflow/security/commit-policy`
   đã always-include — đừng đọc lại.
5. **Ticket package** (change đến từ intake) — `ls docs/extra-docs/<ticket_id>-<slug>/` trước. Task
...
6. **Quality policy** — `.kiro/ai/sonar-policy.md` trước self-review.
...
# S4-FIX — QA tìm ra bug (`/s4-fix {change-name}`)

1. Đọc QA report → bug list (severity + AC-ID) + `tasks.md`.
2. Present fix plan: bảng `| # | Bug | AC-ID | File | Root Cause |`, chờ "go".
3. Theo thứ tự Critical → High → Medium: viết test fail reproduce bug (kèm AC-ID) → fix → regression
   run trên module đó.
4. Type-check + lint sạch, coverage vẫn ≥ threshold. **APPEND** vào dev-test-report.md đang có (KHÔNG
   tạo file mới — QA retest đọc CÙNG file):
   ```
   ## Bug Fixes (S4-FIX — {date})
   | Bug # | AC-ID | Fix | Test | Status |
   ```
5. Exit checklist trước khi trả về QA: type-check/lint/test pass · dev-test-report §Bug Fixes đã
   append · `_decisions.jsonl` có entry `bug_fix` (phase `S4-fix`) · `_handoff.md` regenerate
   (`Generated by: developer`, title `S4-fix → S5-retest`; §1 Bugs Fixed: BUG-{N} ({severity}) đổi gì
   + vì sao đúng · §2 vùng regression risk cho QA retest · §3 reading order) · `_progress.md` updated ·
   `node .kiro/tools/state-set.mjs --set current_phase=S4-FIX --set 'next_action={"agent":"sdlc","command":"approve s4","prerequisite":"Exit checklist done","blocker":"AWAITING BUILD GATE","routes_to":"qa /s5 retest"}'`

# S6 — Release (`/s6 {change-name}`)

1. **Prerequisites** — `_progress.md` cho thấy S5 ✅ GO, QA report 0 Critical/High open. Chưa → STOP.
2. **Migration review** — design.md §DB Schema + mọi migration tạo ở S4: có đủ `up()` + `down()`,
   không có destructive change nào thiếu backup plan.
3. **`{CHANGE_DIR}/release.md`** — fill `release-template.md` qua `deployment-patterns` (release note
   theo AC-ID, migration checklist, rollback plan, post-deploy smoke, deploy strategy).
4. **Archive** — `openspec archive "{change-name}"` (hoặc `/opsx:archive`): merge deltas → living spec,
   move change sang `openspec/changes/archive/`. NEVER merge tay. Archive chạy TRƯỚC deploy thật — có
   chủ đích (giữ living spec tươi cho các spec đang bay khác); RELEASE gate không chờ post-deploy.
5. **Handoff** — `_state.json` (giờ ở `openspec/changes/archive/{change-name}/`), một lệnh:
   `node .kiro/tools/state-set.mjs --change {change-name} --set current_phase=S6 --set 'next_action.agent=null'`
   + một `--set deploy_status.<env>=pending` cho mỗi env promotion thật (theo
   `sdlc.config.json → git.protected_branches`). Nói user review `release.md` rồi deploy.
   Khi từng promotion hoàn tất (ngoài session này): `--set deploy_status.<env>=pass|fail` — breadcrumb,
   không phải gate. Promotion reject: forward-fixable → pipeline `bugfix`/`hotfix` mới, KHÔNG mở lại
   change đã archive; rollback thật → `git revert` commit archive (undo code + living-spec cùng lúc,
   xem `release.md` §"If Rejected After Archive"), never sửa tay living spec.
