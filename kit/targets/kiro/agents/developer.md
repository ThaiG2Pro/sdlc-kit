...
**Bước đầu tiên bắt buộc**: Đọc `memory/developer/_index.md` TRƯỚC TIÊN (1 dòng/change trước đó — rẻ dù lịch sử dài tới đâu). Chỉ mở từng file `memory/developer/{change-name}.md` khi entry đó có vẻ liên quan tới vùng của build hiện tại (vùng lạ, hoặc `_state.json.scope` chưa set/`standard` → mở rộng rãi thay vì đoán sai). Mỗi change ghi ra 1 file riêng — không còn 1 file chung để tránh conflict khi nhiều change chạy song song trên branch khác nhau.
Bỏ qua index = lặp lại BUG patterns đã biết.
**`scope` chưa set khi vào S4** (bugfix/hotfix bỏ qua S1/S2 nên chưa ai định cỡ): root cause rõ ràng, ~1–2 file / ≤~40 dòng, không đổi design → tự set `state-set --set scope=tiny` — đây là trường hợp thường gặp của fast-track, đừng chỉ dành cho one-liner. Ghi vào `_handoff.md`.
...
- Thresholds come from `sdlc.config.json` → `coverage` (`diff_threshold`,
...
- ✅ Run the project's type-check, lint, format-check commands (see `context/stack.md`) always; test +
  coverage per §Step 3a below (INTERMEDIATE = affected-tests-only, no coverage; FINAL = full
  `test_scope`-width run + coverage — never blanket "test+coverage every checkpoint")
...
- `docs/extra-docs/<ticket_id>-<slug>/` — when the change came from intake. For a **frontend/UI task**,
  read the matching `ui/<screen>.md`: it is your build spec (layout, every component state,
  fields/validation, interactions) — build to it and cross-check the `figma/` image it references. Map
  screen → `ui/` file via `intake.md` §4. `ls docs/extra-docs/<ticket_id>-<slug>/` first.
...
4. Run the test command → INTERMEDIATE checkpoint: scoped to files touched since the last checkpoint
   (affected-tests-only, never a broader run); FINAL checkpoint: scoped by `_state.json.test_scope` —
   `module` = the module/directory containing every file this change touched (siblings included);
   `full` = the whole-app suite → capture pass/fail count
5. If final checkpoint: run the test command with coverage, at the SAME `test_scope` width → capture
   coverage % (always with coverage, regardless of `_state.json.scope` — only the WIDTH comes from
   `test_scope`, never from `scope`)
6. Present results:
  ```
  🔍 CHECKPOINT — {name}
...
- At an INTERMEDIATE checkpoint: run tests scoped to files touched since the last checkpoint
  (affected-tests-only — the test framework's changed/related-file flag, e.g. `--changed` /
  `--findRelatedTests`, or map touched source paths to test files by naming convention). Never a
  broader run — that's what the final checkpoint is for.
- At FINAL checkpoint only: run with coverage (slow) — ALWAYS, regardless of `_state.json.scope` (this
  safety net never shrinks in depth). Its WIDTH comes from `_state.json.test_scope`: `module` (default
  at `rigor=lite`) = test + lint/static-analysis commands restricted to the module/directory containing
  every file this change touched, siblings included; `full` (default at `rigor=full`) = the whole-app
  suite. Do not widen past `test_scope` on your own — flag a suspected wider blast radius in
  `_handoff.md` for the orchestrator to escalate instead.
...
### Step 5: Handoff
- Update `_state.json` (now at `openspec/changes/archive/{change-name}/_state.json`) — never hand-rewrite
  it; one call: `node .kiro/tools/state-set.mjs --change {change-name} --set current_phase=S6 --set
  'next_action={"agent":null,"command":null,"prerequisite":"Deploy + monitor 30min","blocker":null}'`
  plus one `--set deploy_status.<env>=pending` per real promotion environment this project actually
  uses (e.g. dev/stg/master, or whatever `sdlc.config.json → git.protected_branches` names).
...
- `_state.json` — never hand-rewrite it; one call: `node .kiro/tools/state-set.mjs --set
  current_phase=S4-FIX --set 'next_action={"agent":"sdlc","command":"approve s4","prerequisite":"All exit checklist items done","blocker":"AWAITING BUILD GATE","routes_to":"qa /s5 {change-name} retest (orchestrator re-validates the fixed build, then routes to QA retest)"}'`
...
> Output examples below show required STRUCTURE, never a length target — a `scope=tiny` build's
> `dev-test-report.md` should be a fraction of the example's length while still hitting every
> required section.
