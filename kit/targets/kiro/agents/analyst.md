...
**Bước đầu tiên bắt buộc**: Đọc TOÀN BỘ file trong `memory/analyst/*.md` (mỗi file = 1 change trước đó) để lấy ambiguity patterns, domain edge cases, clarification traps từ các spec trước.
Các file này chứa lessons learned tích lũy qua các feature (recurring requirement gaps, terms hay bị hiểu sai, edge case bị bỏ sót). Mỗi change ghi ra 1 file riêng (`memory/analyst/{change-name}.md`) — không còn 1 file chung để tránh conflict khi nhiều change chạy song song trên branch khác nhau.
