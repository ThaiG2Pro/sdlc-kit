...
> **Baton budget** — 5 file dưới đây bị đọc TOÀN BỘ mỗi lần spawn, suốt phần còn lại của change. Nên:
> phân tích/ngữ cảnh dài → **proposal.md / spec deltas** (đọc 1 lần ở gate của nó); baton chỉ giữ kết
> luận 1 dòng + con trỏ. Cap: `_handoff.md` ≤6 KB, `_glossary.md` ≤6 KB, `_progress.md` ≤4 KB,
> `_state.json` ≤8 KB chỉ key canonical. cpp-guard báo khi vượt.

- **`_decisions.jsonl`** — ≥1 dòng `"type":"requirement"` (cpp-guard gate S2 yêu cầu). Log **cái spec
  không tự nói được**: AC mà wording bị tranh luận/đổi sau clarification · assumption `[ASSUMED]` +
  nguồn suy ra · BR có ≥2 option (ghi `rejected`). **KHÔNG log 1 dòng/AC** — AC đã nằm trong spec
  deltas mà mọi role đều đọc; log lại chỉ tạo bản sao thứ hai (đo thực tế: 21–28 dòng `requirement`
  cho một change). **Cap mỗi field**: `decision` ≤240 ký tự, `reasoning`/`rejected` ≤120 —
  keyword/fragment, không phải câu văn. **BATCH**: tích lũy trong phiên, ghi GỘP 1 lần Write khi hoàn
  tất phase. Format: `decisions-template.jsonl`.
- **`_glossary.md`** (shape: `glossary-template.md`) — row cho thuật ngữ domain mà **phase sau không đọc
  ra được từ spec** (nghĩa khác nghĩa thông thường, hoặc bị dùng lẫn). Định nghĩa **1 dòng, ≤220 ký
  tự** — cần một đoạn thì đó là design note, không phải glossary. Giữ `Phase` là cột CUỐI (cpp-guard
  đọc nó). Thuật ngữ đổi nghĩa → **SỬA row đó tại chỗ**, không thêm row mới rồi đánh dấu row cũ
  `[SUPERSEDED]` (git có lịch sử). Xóa row chỉ khi thuật ngữ không còn trong spec.
- **`_handoff.md`** (shape: `handoff-template.md`) — header `Generated by: analyst`, title `S2 → S3`,
  ...
  Cộng thêm: quyết định `scope` + lý do 1 dòng (R8b). **THAY nội dung, không cộng dồn**, đúng 5
  section: handoff là "phase sau cần biết gì", không phải lịch sử phase này.
- **`_progress.md`** — dòng S2 của bạn + Next Action. Một row/phase; kể chuyện thì để proposal.md.
- **`_state.json`** — **never rewrite cả file.** Một lệnh `node .kiro/tools/state-set.mjs`:
  `--append phase_history='{"phase":"S2","agent":"analyst","date":"…","note":"…(1-2 câu, ≤200 ký tự; chi tiết → _handoff.md)"}'`
  `--set current_phase=S2` +
  ...
  state-set từ chối; thuật ngữ ở `_glossary.md`, watch item ở `next_action.watch_items`.
