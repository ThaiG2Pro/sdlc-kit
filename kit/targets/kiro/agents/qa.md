...
- **`_decisions.jsonl`** — 1 entry cho MỖI bug. **BATCH**: tích lũy trong phiên, ghi GỘP 1 lần Write
  khi hoàn tất S5 (không phải 1 Write/bug); mỗi dòng ngắn kiểu keyword. Resume sau khi bị kill → GHI
  THÊM phần còn thiếu, không ghi lại từ đầu. Format:
...
- **`_state.json`** — **never rewrite cả file.** Một lệnh `node .kiro/tools/state-set.mjs
  --append phase_history='{"phase":"S5","agent":"qa","date":"…","note":"…(1-3 câu; GO/NO-GO + vì sao)"}'`
