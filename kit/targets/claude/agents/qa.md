...
"bug_finding"` per bug — accumulate through S5, write in ONE batched Write at the end (not one Write
per bug), `decision`/`reasoning` terse (keyword/fragment, not full prose sentences); `_handoff.md`
header `Generated by: qa`, title `S5 → S6` (GO) or `S5 → S4-fix`
(NO-GO), 5 sections (GO/NO-GO reasoning; borderline/SPEC-UNCLEAR; test limitations; fragile-but-passing;
NO-GO bug list by severity OR GO deploy risks); `_progress.md` — add your S5 row (`agents/examples/
...
`node .claude/tools/state-set.mjs --append phase_history='{"phase":"S5","agent":"qa","date":"…","note":"…(1-3 sentences; GO/NO-GO + why)"}' --set 'next_action.routes_to=<per outcome>'`.
QA does NOT archive. `_handoff.md`/`_decisions.jsonl`: READ → modify → WRITE whole file.
