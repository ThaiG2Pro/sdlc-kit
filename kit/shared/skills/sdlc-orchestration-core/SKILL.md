...
> **delegate**: on Claude `spawn` the role subagent (Task); on Kiro tell the user `/agent swap →
> {role}`. Reading the phase docs to "just produce it yourself" is the exact anti-pattern the guard
> stops. `"Generated by: {role}"` in `_handoff.md` must reflect the role that ACTUALLY produced the
...
IF active change AND action = continue:
  ...
  → DELEGATE that phase to its role — do NOT read the phase docs and produce the deliverable
    yourself. On Claude: spawn the role subagent (Task, subagent_type = the phase's agent). On Kiro:
    tell the user "/agent swap → {role}" (you cannot spawn). You only seed the baton + report; the
    role writes proposal/design/tasks/reports. (The write guard will block you if you try anyway.)
...
IF active change AND action = approve:
  ...
  → audit PASS → update _state.json: clear blocker AND set `gates["<current_phase>"]="passed"`,
     mark _progress.md, then DELEGATE the next phase to its role agent (spawn on Claude / `/agent
     swap` on Kiro) — never produce its artifact yourself (the guard printed the next legal phase)
