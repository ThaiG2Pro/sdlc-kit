...
// Standalone mode: write the kit's PERSONAL-layer ignore block to the machine-level git ignore file
// (core.excludesFile, default ~/.config/git/ignore) instead of any repo's committed .gitignore.
...
// Optional kit-owned .gitignore block (opt-in at init). Ignores what the kit regenerates on every
// `init --force` (the per-platform framework dirs + the root config files), PLUS two artifacts that
// are a real, reported source of cross-branch merge conflict:
//   - `memory/<role>/_index.md` — a derived digest, fully regenerable from the `{change-name}.md`
//     files' own `## ` headers (`init --force` backfills it — see `backfillMemoryIndex()`). It's the
//     one memory/ artifact every parallel branch/worktree appends to at the SAME path (unlike
//     `memory/<role>/{change-name}.md` — one unique file per change, never conflicts).
//   - `/context/` — by explicit user decision (not the default recommendation: this content is
//     hand-curated, NOT regenerable, so ignoring it trades "no merge conflict, ever" for "no git
//     history/shared source of truth for it either"). Parallel branches/worktrees that each run
//     onboarder/context-refresh independently were producing real, unresolvable content conflicts on
//     merge; the user judged that risk worse than losing version control on this one directory. Since
//     it's no longer git-tracked, every NEW worktree needs it symlinked in from the main checkout (see
//     `sdlc-orchestration-core` SKILL.md §New Change Setup) — a plain `git worktree add` will not
//     populate an untracked directory.
// `openspec/` and `docs/` are deliberately NOT listed — they hold hand-authored project knowledge /
// work products that SHOULD be committed and shared across branches/team members. Bounded by markers
// so re-init refreshes the block in place (never duplicates) and a user can delete the whole block to
// commit the kit instead.
const GI_BEGIN = '# >>> kiro-sdlc-kit >>>';
const GI_END = '# <<< kiro-sdlc-kit <<<';
// `.kiro/*` (contents) — NEVER `.kiro/` (the directory): ignoring the directory itself stops git
// from descending into it, which kills every `!` negation below it (even ones in a higher-precedence
// layer) and silently hides teammates' NEW `.kiro/specs/` docs — a real, observed data-loss near-miss.
// `!.kiro/specs/` re-opens the one subtree teams commit (legacy Kiro spec format, still in active use).
const GITIGNORE_PATTERNS = ['.claude/', '.kiro/*', '!.kiro/specs/', '/sdlc.config.json', '/pipelines.json', 'memory/*/_index.md', '/context/'];

// PERSONAL-layer block for `--global-ignore` — written to the machine-level git ignore file, never
// to any repo. The "3-layer" rationale: a repo's .gitignore is version-controlled, so a kit block
// there either rots per-branch/worktree while uncommitted (rules vanish on checkout, permanent
// `M .gitignore`) or, once committed, imposes one dev's local-tool setup on the whole team. The
// global layer sidesteps both — and is intrinsically safe to adopt: ignore rules never affect
// files that are already tracked, so teammates' committed docs keep behaving exactly as before.
// Includes everything in GITIGNORE_PATTERNS plus the OpenSpec state ALLOWLIST: ignore everything
// inside a change dir, then re-open only the team deliverables — so every scratch/state file the
// pipeline invents later (_state.json, _handoff.md, run logs, playwright output…) is local
// automatically, with zero rule maintenance.
const GLOBAL_IGNORE_PATTERNS = [
  '# -- kit machinery (regenerated by kiro-sdlc-init --force) --',
  '.claude',                 // no trailing slash: matches a symlinked .claude too
  '.kiro/*',                 // contents, NOT the dir — keeps the negation below alive
  '!.kiro/specs/',           // team-committed Kiro spec docs stay visible
  '.snapshots',              // kit-generated pre-write backups (regenerable)
  '/sdlc.config.json',
  '/pipelines.json',
  'memory/*/_index.md',
  '/context/',
  '',
  '# -- OpenSpec pipeline state: allowlist (ignore all, re-open team deliverables) --',
  '**/openspec/changes/*/*',
  '!**/openspec/changes/*/.openspec.yaml',
  '!**/openspec/changes/*/design.md',
  '!**/openspec/changes/*/proposal.md',
  '!**/openspec/changes/*/tasks.md',
  '!**/openspec/changes/*/openapi.yaml',
  '!**/openspec/changes/*/stride-threat-model.md',
  '!**/openspec/changes/*/specs/',
  '!**/openspec/changes/*/release/',
  '!**/openspec/changes/*/qa/',   // re-open the dir first — git skips ignored dirs entirely
  '**/openspec/changes/*/qa/*',
  '!**/openspec/changes/*/qa/testcases.xlsx',
  '!**/openspec/changes/*/qa/testcases.xls',
  '!**/openspec/changes/*/qa/testcase.xlsx',
  '!**/openspec/changes/*/qa/testcase.xls',
  '!**/openspec/changes/archive/**',  // archived changes are frozen history — keep fully tracked
  '/openspec/_cross-spec-context',
...
function applyTarget(ctx, vals) {
  ...
  const TOOLS_BY_PLATFORM = {
    kiro: ['context-map.mjs', 'context-check.mjs', 'doctor.mjs', 'apply-stack.mjs', 'pipeline-guard.mjs', 'cpp-guard.mjs', 'state-set.mjs', 'state-schema.mjs', 'baton-compact.mjs'],
    claude: ['context-check.mjs', 'doctor-claude.mjs', 'apply-stack.mjs', 'pipeline-guard.mjs', 'cpp-guard.mjs', 'state-set.mjs', 'state-schema.mjs', 'baton-compact.mjs'],
  };
...
// rewritten IN PLACE (same position, byte-for-byte identical surroundings — see below); only a
// first install appends. Everything outside the markers is left untouched; delete the whole block
// to commit the kit instead.
//
// In-place matters because .gitignore is version-controlled and shared. The earlier implementation
// stripped the block and re-appended it at EOF, which silently relocated every user line that sat
// AFTER the block (and trimmed their trailing whitespace) — producing a `M .gitignore` whose diff
// was pure reshuffling. On a repo whose own rules DEPEND on order (`!` negations only win when they
// follow the pattern they override) that reshuffle is not merely noisy, it can change behaviour.
function updateGitignore() {
  ...
  const block = [
    ...
    '# (openspec/, docs/, memory/ are intentionally NOT ignored — commit those. Two exceptions:',
    '# memory/*/_index.md is a regenerable digest that collides across parallel branches; /context/',
    '# is ignored by explicit project choice — symlink it into new worktrees, see SKILL.md.',
    '# .kiro/* ignores kit machinery CONTENTS while !.kiro/specs/ keeps team-committed specs',
    '# visible — never collapse to `.kiro/`: git would stop descending and the negation dies.)',
    ...GITIGNORE_PATTERNS,
    GI_END,
  ].join('\n');
  const si = body.indexOf(GI_BEGIN);
  const ei = body.indexOf(GI_END, si + GI_BEGIN.length);
  let next, where;
  if (si !== -1 && ei !== -1) {
    // Rewrite in place. A duplicate block (only reachable from a hand-edit) is dropped: strip any
    // further marker pairs from the tail, so the file still converges on exactly one.
    const tail = body.slice(ei + GI_END.length)
      .replace(new RegExp(`\\n*${GI_BEGIN}[\\s\\S]*?${GI_END}`, 'g'), '');
    next = body.slice(0, si) + block + tail;
    // Only when the block is the last thing in the file do we own the final byte; anywhere else the
    // tail is the user's and stays byte-for-byte, missing newline included.
    if (!tail) next += '\n';
    where = 'refreshed';
  } else {
    // First install: append. Normalising trailing whitespace is safe here — there is no prior block,
    // so this write happens once and cannot become recurring churn.
    const head = body.replace(/\s*$/, '');
    next = head + (head ? '\n\n' : '') + block + '\n';
    where = existed ? 'appended to' : 'created';
  }
  if (next === body) log('  ✓ .gitignore kit block already up to date');
  else {
    writeFileSync(giPath, next);
    log(`  ✓ ${where} .gitignore (kit block: ${GITIGNORE_PATTERNS.join(', ')})`);
  }
  warnKiroDirShadow(next);
}

// A project line that excludes the `.kiro` DIRECTORY (rather than its contents) silently defeats the
// kit's `!.kiro/specs/` — git never descends into an excluded directory, so no later negation, in any
// precedence layer, can re-include anything under it. Already-tracked specs keep working (ignore rules
// never apply to tracked files), so the damage is invisible until a teammate adds a NEW spec file and
// `git status` simply never mentions it.
//
// We only WARN. The offending line lives outside the markers — it is the project's, often deliberate,
// and rewriting other people's rules is exactly the kind of surprise a tool should not spring on a
// shared, version-controlled file. Print the line number and the one-character fix; let a human decide.
// Position-independent on purpose: an excluded directory is unreachable no matter where the negation
// sits, so a `.kiro/` BELOW the kit block is just as fatal as one above it. The kit's own `.kiro/*`
// never matches here — it names the contents, not the directory.
function warnKiroDirShadow(text) {
  const hits = text.split('\n')
    .map((l, i) => ({ n: i + 1, raw: l.trim() }))
    .filter(({ raw }) => /^\/?\.kiro\/?$/.test(raw));
  if (!hits.length) return;
  log('  ! .gitignore excludes the .kiro DIRECTORY above the kit block — `!.kiro/specs/` cannot work:');
  for (const h of hits) log(`      line ${h.n}: ${h.raw}`);
  log('    git stops descending into an excluded directory, so NEW .kiro/specs/ files teammates add');
  log('    will never show up in `git status` (already-committed ones are unaffected).');
  log('    Fix by hand — the kit will not touch a line outside its own markers:');
  log(`      ${hits[0].raw}  →  ${hits[0].raw.replace(/\/?$/, '/*')}`);
}

// Where git reads the machine-level ignore file: explicit core.excludesFile wins; otherwise git's
// documented default $XDG_CONFIG_HOME/git/ignore (falling back to ~/.config/git/ignore).
function resolveGlobalIgnorePath() {
  try {
    const p = execSync('git config --global --get core.excludesFile', { stdio: ['ignore', 'pipe', 'ignore'] })
      .toString().trim();
    if (p) return p.replace(/^~(?=$|[\/\\])/, homedir());
  } catch { /* unset → git default below */ }
  return join(env.XDG_CONFIG_HOME || join(homedir(), '.config'), 'git', 'ignore');
}

// Idempotently maintain the kit-owned block in the MACHINE-level git ignore file (same marker
// strategy as updateGitignore, different file). Everything outside the markers is left untouched.
function updateGlobalIgnore() {
  const giPath = resolveGlobalIgnorePath();
  let body = '', existed = false;
  try { body = readFileSync(giPath, 'utf8'); existed = true; } catch { /* no file yet */ }
  const re = new RegExp(`\\n*${GI_BEGIN}[\\s\\S]*?${GI_END}\\n?`, 'g');
  const stripped = body.replace(re, '');
  const block = [
    GI_BEGIN,
    '# Kit-owned PERSONAL layer: refresh with `kiro-sdlc-init --global-ignore`. Applies to every',
    '# repo on this machine (all branches/worktrees/future clones); never committed, never on a',
    '# remote. Ignore rules do NOT affect already-tracked files, so teammates\' committed docs',
    '# (.kiro/specs/, existing openspec deliverables) keep behaving exactly as before.',
    ...GLOBAL_IGNORE_PATTERNS,
    GI_END,
  ].join('\n');
  const next = stripped.replace(/\s*$/, '') + (stripped.trim() ? '\n\n' : '') + block + '\n';
  if (next === body) { log(`  ✓ global ignore block already up to date (${giPath})`); return; }
  mkdirSync(dirname(giPath), { recursive: true });
  writeFileSync(giPath, next);
  log(`  ✓ ${existed ? 'updated' : 'created'} ${giPath} (kit personal-layer block)`);
  log('    • Covers every repo on this machine — you can now answer "n" to the per-repo .gitignore');
  log('      question at init (or pass --no-gitignore) and keep team .gitignore files untouched.');
  log('    • Ignore never affects tracked files. If a repo already TRACKS kit machinery, untrack it');
  log('      selectively (never sweep team docs): git ls-files .claude .kiro | review first, then');
  log('      git rm -r --cached .claude — but keep .kiro/specs/ and any team-committed files.');
}
