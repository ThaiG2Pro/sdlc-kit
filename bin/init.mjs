...
// `init --force` (the per-platform framework dirs + the root config files), PLUS `_index.md` — a
// derived digest under `memory/<role>/`, fully regenerable from the `{change-name}.md` files' own
// `## ` headers (`init --force` backfills it — see `backfillMemoryIndex()`). It's the one memory/
// artifact multiple parallel branches/worktrees actually collide on (every branch appends to the SAME
// file, unlike `memory/<role>/{change-name}.md` — one unique file per change, never conflicts). The
// REST of context/, openspec/, docs/, and memory/ are deliberately NOT listed — they hold
// hand-authored project knowledge / work products that SHOULD be committed and shared across
// branches/team members. Bounded by markers so re-init refreshes the block in place (never
// duplicates) and a user can delete the whole block to commit the kit instead.
const GI_BEGIN = '# >>> kiro-sdlc-kit >>>';
const GI_END = '# <<< kiro-sdlc-kit <<<';
const GITIGNORE_PATTERNS = ['.claude/', '.kiro/', '/sdlc.config.json', '/pipelines.json', 'memory/*/_index.md'];
...
function updateGitignore() {
  ...
  const block = [
    ...
    '# (context/, openspec/, docs/, memory/ are intentionally NOT ignored — commit those. Exception:',
    '# memory/*/_index.md is a regenerable digest that collides across parallel branches — ignored.)',
    ...GITIGNORE_PATTERNS,
    GI_END,
  ].join('\n');
